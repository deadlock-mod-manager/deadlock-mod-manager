...
If no relevant information is available in the skill's references directory, prefer the project's tailored `security-review` skill for Tauri/Hono/Rust reviews. Otherwise think about well-known security best practices for the language and framework in scope.

## This project

Stack: Tauri 2 + Rust desktop backend, React/TypeScript webviews (`apps/desktop`, `apps/www`), Bun + Hono API (`apps/api`), Better Auth (`apps/auth`), Fumadocs/Next.js docs site (`apps/docs`).

For this repo, load only these references when applicable:

- `references/javascript-general-web-frontend-security.md` — shared frontend guidance
- `references/javascript-typescript-react-web-frontend-security.md` — desktop/www React apps
- `references/javascript-typescript-nextjs-web-server-security.md` — `apps/docs` only

For Tauri IPC, Rust backend, Hono/oRPC API, and auth boundary reviews, use `.agents/skills/security-review/` instead of the generic backend references in this skill.
