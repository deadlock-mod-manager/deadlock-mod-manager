...
const result = await db.select().from(mods).where(eq(mods.name, userInput));
...
const user = await db.select().from(users).where(userInput); // if userInput is a crafted SQL expression
...
export const searchMods = publicProcedure.mutation(
