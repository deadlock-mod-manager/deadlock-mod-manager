...
- **Service APIs**: Profiles, reports, policies, server discovery, and VPK analysis
...
The public API no longer mirrors the GameBanana catalog. The desktop app owns a
local catalog for responsive and offline browsing, while the API remains the
source of project policy and other Deadlock Mod Manager services.
