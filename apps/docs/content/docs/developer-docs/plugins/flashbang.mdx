...
<Callout type='warning'>
  **Theme Not Switching** - Verify theme provider implements
  `setFlashbangEnabled` - Check localStorage values are being read correctly -
  Ensure plugin enable state is properly synchronized
</Callout>

<Callout type='warning'>
  **Settings Not Persisting** - Check `usePersistedStore` integration - Verify
  localStorage synchronization in both Settings and Render components - Ensure
  input validation doesn't prevent valid values
</Callout>
