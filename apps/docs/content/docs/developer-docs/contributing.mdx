...
2. **Set up your development environment**: Follow the [Development Setup](/developer-docs/development-setup) guide
3. **Understand the project structure**: Review the [Project Structure](/developer-docs/project-structure) documentation
...
All translations are managed on [Crowdin](https://translate.deadlockmods.app/). The English source (`apps/desktop/src/locales/en.json`) is the only locale file edited directly in the repo - every other locale is synced back from Crowdin automatically via the [Crowdin GitHub action](https://github.com/deadlock-mod-manager/deadlock-mod-manager/blob/main/.github/workflows/crowdin.yml).

<Callout type='info' title='Where to translate'>
  Contribute translations at
  [translate.deadlockmods.app](https://translate.deadlockmods.app/). Do not edit
  non-English JSON files in the repo directly - they will be overwritten on the
  next Crowdin sync.
</Callout>
...
3. Approved translations are proposed as a PR on this repo automatically

### Requesting a New Language

Request a new language on Crowdin, or [file an issue](https://github.com/deadlock-mod-manager/deadlock-mod-manager/issues/new) and coordinate in the [#translations](https://discord.com/channels/1322369530386710568/1414203136939135067) Discord channel.

### Adding or Changing Source Strings

When adding new UI strings in a PR, add them to `apps/desktop/src/locales/en.json`. Once your PR is merged to `main`, the Crowdin action uploads the new source strings for translators.
