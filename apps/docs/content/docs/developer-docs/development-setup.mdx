...
| Command                      | Description                                           |
| ---------------------------- | ----------------------------------------------------- |
| `pnpm dev`                   | Run the repository-wide turbo dev workflow            |
...
Desktop development starts with the local UI tooling bridge enabled. The bridge
listens on `127.0.0.1` and uses a development-only Tauri config so the global
Tauri API is not enabled in production builds.
