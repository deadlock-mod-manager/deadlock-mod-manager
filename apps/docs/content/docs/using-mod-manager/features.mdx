...
- **Unified Friend List**: View accepted friends alongside incoming and outgoing requests in one place
- **Request Workflow**: Send, accept, decline, or cancel requests without leaving the app
- **Contextual Profiles**: See display names and avatars for every connection
- **Real-Time Sync**: Updates reflect immediately after each action so everyone sees the current state
