    ...
    endpoints: {
      ...
      steamCallback: createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          const requestSignUp = requestSignUpString === "true";
          ...
          if (!isMatch(callbackOrigin)) {
            ctx.context.logger.error(
              "Steam callback URL not in trusted origins",
              {
                callbackURL,
                callbackOrigin,
                trustedOrigins,
          ...
          const newUserCallbackOrigin = new URL(newUserCallbackURL).origin;
          if (!isMatch(newUserCallbackOrigin)) {
            ctx.context.logger.error(
              "Steam new user callback URL not in trusted origins",
              {
                newUserCallbackURL,
                newUserCallbackOrigin,
                trustedOrigins,
              },
            );
            throw ctx.redirect(
          ...
          const errorCallbackOrigin = new URL(errorCallbackURL).origin;
          if (!isMatch(errorCallbackOrigin)) {
            ctx.context.logger.error(
              "Steam error callback URL not in trusted origins",
              {
                errorCallbackURL,
                errorCallbackOrigin,
                trustedOrigins,
