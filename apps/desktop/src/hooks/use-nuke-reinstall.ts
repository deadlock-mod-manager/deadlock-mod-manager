...
/** Local mods only exist on disk, so a nuke would destroy them for good. */
const isLocalMod = (mod: Pick<LocalMod, "remoteId">) =>
...
export const useNukeReinstall = () => {
  ...
  const run = useCallback(async () => {
    ...
    for (const [index, mod] of targets.entries()) {
      ...
      setProgress();
      patchMod(mod.remoteId, { outcome: result.outcome, error: result.error });
    }
