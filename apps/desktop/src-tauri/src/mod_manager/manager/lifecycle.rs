...
impl ModManager {
  pub fn install_mod(
    ...
    deadlock_mod.installed_vpks = pending.value().clone();
    ...
    if let Err(save_error) = manifest.save(&addons_path) {
      return Err(pending.rollback(save_error).into());
    }
    pending.commit();
  ...
  pub fn uninstall_mod(
    ...
    let pending = ops::disable_vpks_in(
      &enabled_dir,
      &addons_path,
      &mod_id,
      &installed_vpks,
    ...
    let prefixed_vpks = pending.value().clone();
  ...
  pub fn backfill_fingerprints(
    ...
    budget: usize,
  ) -> Result<vpkmanager::backfill::BackfillOutcome, Error> {
    let addons_path = self.get_addons_path(profile_folder)?;
    if !addons_path.exists() {
      return Ok(vpkmanager::backfill::BackfillOutcome::default());
    }
  ...
  pub fn replace_mod_vpks(
    ...
    let pending = ops::stage_replace_vpks(
      &addons_path,
      &enabled_dir,
      &mod_id,
      &source_vpk_paths,
    ...
    let mut manifest = match ProfileLedger::load(&addons_path) {
      Ok(manifest) => manifest,
      Err(error) => return Err(pending.rollback(error).into()),
    };
    if installed_vpks.is_empty() {
      let prefixed_vpks = match ops::find_prefixed_vpks(&addons_path, &mod_id) {
        Ok(prefixed_vpks) => prefixed_vpks,
        Err(error) => return Err(pending.rollback(error).into()),
      };
