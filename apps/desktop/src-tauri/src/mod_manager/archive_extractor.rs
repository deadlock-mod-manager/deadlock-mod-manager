...
impl ArchiveExtractor {
    ...
    fn sanitize_path(&self, path: &str, output_dir: &Path) -> Result<std::path::PathBuf, Error> {
        ...
        let canonical_output = output_dir.canonicalize().map_err(|_| {
            Error::UnauthorizedPath("Unable to resolve output directory".to_string())
        })?;
        ...
        if check_path.exists() {
            let canonical_check = check_path.canonicalize().map_err(|_| {
                Error::UnauthorizedPath(format!("Unable to resolve path: {}", path))
            })?;
