...
pub async fn set_discord_presence(
  ...
  if needs_new_client {
    ...
    if connect_attempts >= max_connect_attempts {
      log::error!("Failed to connect to Discord after {} attempts", max_connect_attempts);
      log::info!("Discord might not be running or the Application ID might be invalid");
      return Err(Error::InvalidInput("Failed to connect to Discord. Make sure Discord is running and you're logged in.".to_string()));
    }
  }
  ...
  while attempts < max_attempts {
    ...
    match result {
      ...
      Err(e) => {
        ...
        let should_reconnect = e.contains("pipe") || e.contains("Pipe") || e.contains("os error 232") || e.contains("Broken pipe");
        if should_reconnect {
  ...
  log::error!("Failed to set Discord presence after {} attempts", max_attempts);
  Err(Error::InvalidInput("Failed to set Discord presence after multiple attempts".to_string()))
}
