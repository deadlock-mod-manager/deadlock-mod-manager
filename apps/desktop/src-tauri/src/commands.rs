...
pub async fn set_language(app_handle: AppHandle, language: String) -> Result<(), Error> {
  ...
  let supported_languages = [
    ...
    "es-ES", "pt-BR", "it-IT", "ja-JP", "ko-KR", "bg-BG",
  ];
...
// ============================================================================
// Server Addons Commands
// ============================================================================

const SERVER_FOLDER_PREFIX: &str = "server_";

/// Sanitize a server identifier into a filesystem-safe folder slug.
fn sanitize_server_id(server_id: &str) -> Result<String, Error> {
...
/// Reject path-traversal and folders without the `profile_`/`server_` prefix.
fn validate_addons_subfolder(folder_name: &str) -> Result<(), Error> {
...
pub async fn apply_server_gameinfo(
  ...
  // Server folder takes precedence (listed first), then optionally the active
  // profile so the user's cosmetics still load when "layered" mode is on.
...
pub async fn cleanup_stale_server_gameinfo(
  ...
  // If we weren't told which profile to restore but the file already
  // references one, recover it instead of dropping back to bare addons —
  // protects against being called before the frontend store has hydrated.
...
/// Hard cap on a single custom-provider download (512 MiB).
const CUSTOM_PROVIDER_MAX_BYTES: u64 = 512 * 1024 * 1024;

/// Enforced at the Tauri boundary so the UI guard can't be bypassed.
...
/// SSRF guard for custom-provider URLs from untrusted server listings:
