    ...
    F: Fn(DownloadProgress) + Send + 'static,
{
    ...
    let response = client.get(url).send().await.map_err(|e| {
        Error::NetworkError(format!("Failed to send request: {}", e))
    })?;
    ...
    let mut file = File::create(target_path).await.map_err(|e| {
        Error::FileWriteFailed(format!("Failed to create file: {}", e))
    })?;
    ...
    while let Some(chunk) = stream.next().await {
        ...
        let chunk = chunk.map_err(|e| {
            Error::NetworkError(format!("Failed to read chunk: {}", e))
        })?;

        file.write_all(&chunk).await.map_err(|e| {
            Error::FileWriteFailed(format!("Failed to write to file: {}", e))
        })?;
    ...
    file.flush().await.map_err(|e| {
        Error::FileWriteFailed(format!("Failed to flush file: {}", e))
    })?;
