use crate::game_state_watcher::patterns::PATTERNS;
...
impl LogParser {
  ...
  pub fn process_line(&mut self, line: &str, state: &mut GameState) -> bool {
    ...
    if let Some(caps) = p.party_event.captures(line) {
      ...
      if matches!(
        state.phase,
        GamePhase::MainMenu | GamePhase::Hideout | GamePhase::PartyHideout | GamePhase::InQueue
      ) {
      ...
      if reason.to_uppercase().contains("EXITING") {
        ...
        state.reset();
      } else if !reason.to_uppercase().contains("LOOPDEACTIVATE")
        && matches!(
          state.phase,
      ...
      )
    {
      ...
      if state.phase != GamePhase::Spectating && !in_hideout_map && !self.hideout_loaded {
        ...
        if state_name == "matchintro" || state_id == 4 {
          state.enter_match_intro();
        } else if state_name == "gameinprogress" || state_name == "inprogress" || state_id == 7 {
  ...
  fn apply_hero_signal(&mut self, state: &mut GameState, hero_key: &str) {
    let hero_norm = hero_key.to_lowercase().replace("hero_", "");
