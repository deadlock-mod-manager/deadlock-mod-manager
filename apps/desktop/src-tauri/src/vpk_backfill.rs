...
impl Drop for RunningSlot {
  fn drop(&mut self) {
    running_set().remove(&self.0);
  }
}

fn running_set() -> std::sync::MutexGuard<'static, HashSet<String>> {
  RUNNING.lock().unwrap_or_else(|poisoned| {
    log::warn!("Fingerprint backfill running-set lock was poisoned; recovering");
    poisoned.into_inner()
  })
}
...
pub fn spawn_for_profile(profile_folder: Option<String>) {
  ...
  if !running_set().insert(key.clone()) {
    return;
  }
...
async fn run(profile_folder: Option<String>) {
  for _ in 0..MAX_BATCHES {
    let outcome = match MANAGER.lock() {
      Ok(manager) => manager.backfill_fingerprints(profile_folder.as_deref(), BATCH_SIZE),
      Err(_) => {
        log::warn!("Stopped fingerprinting profile {profile_folder:?}: manager lock was poisoned");
        return;
      }
    };

    match outcome {
      Ok(outcome) if outcome.stamped == 0 && !outcome.failed.is_empty() => {
        log::warn!(
          "Stopped fingerprinting profile {profile_folder:?}: no progress after {} failure(s)",
          outcome.failed.len()
        );
        return;
      }
