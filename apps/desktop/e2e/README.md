...
M6 is implemented in `.github/workflows/desktop-e2e.yml` and awaits clean-runner qualification. Desktop-related PRs build the harness once and run `ci-pr` serially: About, local import/delete, Range recovery, interrupted downloads, GameBanana variants, and committed-transaction recovery. Scheduled runs at 03:23 UTC and manual dispatch expand to one independent Windows job per scenario family. Apply the `e2e-full` PR label to run that complete matrix and installed-package smoke before merge; later pushes retain full coverage while the label is present. Scenarios within each job remain serial, with retries disabled. Native input is explicitly enabled only on these disposable runner desktops.

Every triggered run also builds an ordinary release without `e2e-harness` or the frontend harness flag. The release check rejects WDIO dependencies in the resolved Cargo tree, control markers in ASCII/UTF-16 executable content and frontend assets, and harness permissions in generated capabilities. It then passes E2E configuration to the ordinary executable and requires the production rejection message and exit code 2 before app startup. This checks exclusion of the E2E server/control surface and prevents the fixture policy from being activated in production; it is not a general audit of every production network integration.

Nightly/manual runs additionally build an unsigned NSIS package with a distinct E2E product/identifier and no production deep-link schemes. A runner installs it into a unique temporary directory, compares the installed executable's hash with the build output, and runs About plus the native local-mod lifecycle against that installed path before uninstalling. This covers release-mode packaging with instrumentation; ordinary release exclusion is checked separately. Neither probe can install or launch a release on a developer desktop: both require GitHub-hosted CI environment guards. No signing secrets or release publication are used.

The CLI's `--report <path>` writes a JSON suite summary after each completed case. CI retains these reports and each world's diagnostic artifacts for 14 days, including on failures; it does not upload world control tokens. Harness binaries are retained for three days for the downstream scenario jobs. A failed runner prerequisite check, missing installer, unmatched request, timeout, or failed scenario remains a failing check. Do not treat M6 as qualified until the workflow has passed on clean runners. The local pointer rerun was explicitly skipped at the user's request; this milestone does not claim it passed.
...
| M6: CI and release coverage | Implemented; clean-runner qualification pending | Serial Windows PR lane, nightly family matrix, installed NSIS/native smoke, and ordinary-release exclusion checks | Clean runners reproduce required flows and ordinary builds contain no harness server, capability, control channel, or active fixture policy |
