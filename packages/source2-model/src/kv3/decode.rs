...
pub(crate) fn read_i16(data: &[u8], pos: usize) -> Result<i16> {
    data.get(pos..pos + 2)
...
pub(crate) fn read_u64(data: &[u8], pos: usize) -> Result<u64> {
    data.get(pos..pos + 8)
        .map(|b| u64::from_le_bytes(b.try_into().unwrap()))
        .ok_or_else(|| Source2Error::Resource(format!("KV3 u64 read out of bounds at {pos}")))
}

pub(crate) fn read_i64(data: &[u8], pos: usize) -> Result<i64> {
    data.get(pos..pos + 8)
...
pub(crate) fn read_f64(data: &[u8], pos: usize) -> Result<f64> {
    data.get(pos..pos + 8)
        .map(|b| f64::from_le_bytes(b.try_into().unwrap()))
        .ok_or_else(|| Source2Error::Resource(format!("KV3 f64 read out of bounds at {pos}")))
}

pub(crate) fn align(pos: &mut usize, alignment: usize) {
...
pub(crate) fn read_null_string(bytes: &[u8], pos: &mut usize) -> Result<String> {
    let start = *pos;
...
pub(crate) fn decompress_lz4(input: &[u8], output_len: usize) -> Result<Vec<u8>> {
    ...
    if written != output_len {
        return Err(Source2Error::Decode(format!(
            "KV3 LZ4 decoded {written} bytes, expected {output_len}"
        )));
    }
    Ok(output)
}

pub(crate) fn decompress_lz4_chain(
    input: &[u8],
    ...
    for &compressed_size in compressed_sizes {
        ...
        let compressed = input
            .get(input_pos..input_pos.saturating_add(compressed_size))
            .ok_or_else(|| {
                Source2Error::Resource("KV3 compressed LZ4 chain block out of bounds".into())
            })?;
        let decoded_size = frame_size.min(output_len - output.len());
        let dictionary_start = output.len().saturating_sub(64 * 1024);
        let mut decoded = vec![0u8; decoded_size];
        let written =
            decompress_into_with_dict(compressed, &mut decoded, &output[dictionary_start..])
                .map_err(|error| {
                    Source2Error::Decode(format!("KV3 chained LZ4 decode failed: {error}"))
                })?;
        if written == 0 || written > decoded_size {
            return Err(Source2Error::Decode(format!(
                "KV3 chained LZ4 decoded invalid frame size {written} (capacity {decoded_size})"
            )));
        }
        output.extend_from_slice(&decoded[..written]);
        input_pos += compressed_size;
    }

    if output.len() != output_len {
        return Err(Source2Error::Decode(format!(
            "KV3 chained LZ4 decoded {} bytes, expected {output_len}",
            output.len()
        )));
    }
    Ok(output)
...
pub(crate) fn decompress_zstd(input: &[u8], output_len: usize) -> Result<Vec<u8>> {
    zstd::bulk::decompress(input, output_len)
        .map_err(|e| Source2Error::Decode(format!("KV3 ZSTD decode failed: {e}")))
}

pub(crate) fn read_type(context: &mut KvContext) -> Result<NodeType> {
...
pub(crate) fn string_by_id(context: &KvContext, id: i32) -> Result<String> {
    if id == -1 {
...
pub(crate) fn read_object_len(context: &mut KvContext) -> Result<usize> {
...
pub(crate) fn read_value(context: &mut KvContext, node_type: NodeType) -> Result<KvValue> {
    match node_type {
...
pub(crate) fn read_array_items(context: &mut KvContext, len: usize) -> Result<KvValue> {
    let mut values = Vec::with_capacity(len);
    for _ in 0..len {
...
pub(crate) fn read_object(context: &mut KvContext) -> Result<KvValue> {
...
pub fn parse(data: &[u8]) -> Result<KvValue> {
    ...
    let mut size_block_compressed_sizes_bytes = 0usize;
    let (
        count_bytes1,
        count_bytes4,
        count_bytes8,
        ...
        if version >= 4 {
            ...
            size_block_compressed_sizes_bytes = read_i32(data, pos + 4)? as usize;
    ...
    if version >= 5 {
        ...
        if count_blocks > 0 {
            ...
            let size_end = if size_block_compressed_sizes_bytes > 0 {
                size_pos
                    .saturating_add(size_block_compressed_sizes_bytes)
                    .min(buffer2.len())
            } else {
                buffer2.len()
            };
            while size_pos + 2 <= size_end {
    ...
    if count_blocks > 0 && size_binary_blobs > 0 {
        binary_blobs = match compression_method {
            0 => data
                ...
                .to_vec(),
            1 => {
                let compressed_size = blob_block_sizes.iter().sum::<usize>();
                let compressed = data
