...
pub fn run(ledger: &mut ProfileLedger, pending: &[PendingStamp], budget: usize) -> BackfillOutcome {
    ...
    // A file we could not parse is not a VPK we can stamp; leave it for a human
    // to look at rather than rewriting it.
    let mut claimed = pending.iter().filter_map(|file| {
        let mod_id = file.mod_id.as_deref()?;
        (file.reason != StampReason::UnreadableStamp && file.reason != StampReason::ForeignVersion)
            .then_some((file, mod_id))
    });

    for (file, mod_id) in claimed.by_ref().take(budget) {
        ...
        match fingerprint::stamp(&file.path, mod_id, &file.original_name) {
            Ok(fingerprint) => match track_stamped(ledger, file, &fingerprint) {
                ...
                Err(error) => {
                    log::warn!(
                        "Stamped {} but could not track it: {error}",
                        file.path.display()
                    );
                    outcome.failed.push((
                        file.path.clone(),
