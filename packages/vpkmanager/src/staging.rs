//! Making a multi-file VPK operation survive a failure halfway through.
//!
//! Every mutation in [`crate::ops`] moves or rewrites several files, and any one
//! of those steps can fail — the game holds a pak file open, the disk fills, the
//! process is killed. The three transactions here cover the three shapes that
//! takes: files moved aside into a staging directory ([`VpkStaging`]), files
//! renamed in place ([`RenameTransaction`]), and files overwritten in place
//! ([`VpkSnapshot`]). All of them undo themselves on drop unless committed, and
//! the staging directory is written so that even a hard crash leaves something
//! [`recover_staging_directory`] can put back.
...
use crate::fs_retry;
use crate::naming;
use crate::profile::{ProfileBase, StagedName};

/// An error type a transaction can report a failed rollback through.
///
/// Rolling back returns the error that caused it, so that the caller's own
/// error type — and the error kind its UI branches on — survives the trip
/// through a transaction it did not define.
pub trait RollbackError: std::fmt::Display {
    /// The error to report when the rollback *itself* failed, leaving files in
    /// a state neither side describes.
    fn rollback_failed(message: String) -> Self;
}

impl RollbackError for VpkManagerError {
    fn rollback_failed(message: String) -> Self {
        VpkManagerError::RollbackFailed(message)
    }
}
...
pub fn recover_staging_directory(
    ...
    for entry in fs::read_dir(staging_dir)? {
        ...
        let staged_name = staged_path
            ...
            .ok_or_else(|| VpkManagerError::Vpk("Invalid VPK staging filename".to_string()))?;
        ...
        if destination.exists() {
            match mode {
                ...
                RecoveryMode::AvoidEnabledCollisions => {
                    destination =
                        destination_dir.join(naming::next_free_enabled_vpk_name(&destination_dir)?);
                }
            }
        }
        fs_retry::rename(staged_path, destination)?;
    }
...
/// next ledger load recovers them via [`recover_staging_directory`].
...
impl VpkStaging {
    pub fn claim(base: &ProfileBase, name: &str) -> Result<Self> {
    ...
    pub fn stage(&mut self, base: &ProfileBase, source: &Path) -> Result<PathBuf> {
        let shard = source
            .parent()
        ...
        let filename = source
            .file_name()
            .and_then(|name| name.to_str())
            .ok_or_else(|| VpkManagerError::Vpk("VPK filename is not valid UTF-8".to_string()))?;
        let staged = self.dir.join(StagedName::new(shard, filename).encode());
        fs_retry::rename(source, &staged)?;
    ...
    pub fn place(&mut self, staged: &Path, destination: &Path) -> Result<()> {
        if destination.exists() {
            return Err(VpkManagerError::Vpk(format!(
        ...
        fs_retry::rename(staged, destination)?;
        let file = self
            .files
            .iter_mut()
            .find(|file| file.current == staged)
            .ok_or_else(|| {
                VpkManagerError::Vpk(format!("Untracked staged VPK: {}", staged.display()))
            })?;
    ...
    pub fn rollback<E: RollbackError>(mut self, original_error: E) -> E {
    ...
    fn restore_files(&mut self) -> Vec<String> {
        ...
        for file in self.files.iter_mut() {
            ...
            if let Err(error) = fs_retry::rename(&file.current, &file.parked) {
        ...
        for file in self.files.iter().rev() {
            ...
            if let Err(error) = fs_retry::rename(&file.current, &file.original) {
                failures.push(format!(
                    "{} -> {}: {error}",
                    file.current.display(),
...
enum PendingGuard {
    ...
    Rename(RenameTransaction),
}

/// A completed filesystem mutation that remains reversible until its ledger
/// commit succeeds. Holds every guard the operation produced so a swap can
/// absorb enable and disable without a second, hand-rolled rollback.
...
impl<T> PendingVpkOperation<T> {
    ...
    pub(crate) fn with_staging(value: T, staging: VpkStaging) -> Self {
        Self {
            value,
            guards: vec![PendingGuard::Staging(staging)],
        }
    }

    pub(crate) fn with_snapshot(value: T, snapshot: VpkSnapshot) -> Self {
        Self {
            value,
            guards: vec![PendingGuard::Snapshot(snapshot)],
        }
    }

    pub(crate) fn with_rename(value: T, rename: RenameTransaction) -> Self {
        Self {
            value,
            guards: vec![PendingGuard::Rename(rename)],
        }
    }

    /// Keep this operation's value and take ownership of another operation's
    /// guards, so two sequential mutations roll back as one.
    pub(crate) fn absorb<U>(mut self, other: PendingVpkOperation<U>) -> Self {
        self.guards.extend(other.guards);
        self
    }

    pub(crate) fn map_value<U>(self, value: U) -> PendingVpkOperation<U> {
        PendingVpkOperation {
            value,
            guards: self.guards,
        }
    ...
    pub fn commit(self) -> T {
        for guard in self.guards {
            match guard {
                ...
                PendingGuard::Rename(rename) => rename.commit(),
            }
        }
        self.value
    }

    pub fn rollback<E: RollbackError>(self, original_error: E) -> E {
        let mut error = original_error;
        for guard in self.guards.into_iter().rev() {
            error = match guard {
                PendingGuard::Staging(staging) => staging.rollback(error),
                PendingGuard::Snapshot(snapshot) => snapshot.rollback(error),
                PendingGuard::Rename(rename) => rename.rollback(error),
            };
        }
        error
    }
...
impl<T> fmt::Debug for PendingVpkOperation<T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("PendingVpkOperation")
...
impl RenameTransaction {
    ...
    fn restore(&self) -> Vec<String> {
        ...
        for (current, original) in self.renamed.iter().rev() {
            if let Err(error) = fs_retry::rename(current, original) {
                failures.push(format!(
                    "{} -> {}: {error}",
                    current.display(),
                    original.display()
    ...
    fn restore(&self) -> Vec<String> {
        ...
        for path in &self.touched {
            if !backed_up.contains(path)
                && path.exists()
                && let Err(error) = fs_retry::remove_file(path)
            {
        ...
        for (destination, backup) in &self.originals {
            ...
            if let Err(error) = fs_retry::copy(backup, destination) {
                failures.push(format!(
...
mod tests {
    ...
    fn rollback_restores_staged_and_placed_files() {
        ...
        let placed = base
            .shard_dir(ShardIndex::new(2).unwrap())
            .join("pak01_dir.vpk");
        staging.place(&staged, &placed).unwrap();

        let error = staging.rollback(VpkManagerError::Invalid("commit failed".to_string()));
    ...
    #[test]
    fn pending_operation_rolls_back_when_ledger_commit_fails() {
        let temp = tempfile::tempdir().unwrap();
        let base = base(&temp);
        let original = base.join("pak01_dir.vpk");
        fs::write(&original, b"vpk").unwrap();
        ...
        let error = pending.rollback(VpkManagerError::Invalid("ledger failed".to_string()));
    ...
    fn snapshot_restores_originals_and_removes_new_files() {
        ...
        let error = snapshot.rollback(VpkManagerError::Invalid("commit failed".to_string()));
    ...
    fn snapshot_removes_files_captured_before_they_existed() {
        ...
        snapshot.rollback(VpkManagerError::Invalid("commit failed".to_string()));
    ...
    /// Recovery must never overwrite a file that already claimed the slot: a
    /// reorder interrupted mid-placement leaves exactly that collision.
    #[test]
    fn recovery_does_not_clobber_already_placed_files() {
        let temp = tempfile::tempdir().unwrap();
        let base = base(&temp);
        fs::write(base.join("pak01_dir.vpk"), b"placed").unwrap();
        let reorder = base.join(crate::profile::REORDER_STAGING_DIR);
