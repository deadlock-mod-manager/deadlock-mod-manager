...
impl ProfileLedger {
    ...
    /// Record where a fingerprinted file currently is. Returns whether the
    /// stored identity, location or stat cache actually changed — `last_seen`
    /// alone does not count, so a clean rescan does not dirty the ledger.
    pub fn track(&mut self, fingerprint_id: impl Into<String>, tracked: TrackedVpk) -> bool {
        let id = fingerprint_id.into();
        if let Some(existing) = self.files.get(&id)
            && existing.mod_id == tracked.mod_id
            && existing.shard == tracked.shard
            && existing.filename == tracked.filename
            && existing.state == tracked.state
            && existing.original_name == tracked.original_name
            && existing.content_hash == tracked.content_hash
            && existing.size == tracked.size
            && existing.modified_at == tracked.modified_at
        {
            if let Some(existing) = self.files.get_mut(&id) {
                existing.last_seen = tracked.last_seen;
            }
            return false;
        }
...
mod tests {
  ...
  "mods": {
    ...
    #[test]
    fn track_refreshes_last_seen_without_dirtying_a_clean_record() {
        let mut ledger = ProfileLedger::default();
        let tracked = TrackedVpk {
            mod_id: "123".to_string(),
            shard: ShardIndex::FIRST,
            filename: "pak01_dir.vpk".to_string(),
            state: VpkState::Enabled,
        ...
        assert!(ledger.track("fingerprint-1", tracked.clone()));

        let mut later = tracked;
        later.last_seen = 99;
        assert!(!ledger.track("fingerprint-1", later));
        assert_eq!(ledger.tracked("fingerprint-1").unwrap().last_seen, 99);
    }
