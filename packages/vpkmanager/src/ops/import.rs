...
use crate::staging::{PendingVpkOperation, VpkSnapshot};
...
/// `None` copies every VPK found. A selection that cannot be resolved in full
/// is rejected before anything is copied, because callers treat a successful
/// return as licence to delete the extracted directory and its archive.
pub fn copy_vpks_with_prefix(
    ...
    let sources = resolve_copy_sources(source_dir, mod_id, selected)?;
    copy_resolved_vpks_with_prefix(&sources, destination_dir, mod_id)
}

/// Copy VPKs into a profile under the `{mod_id}_` prefix, remaining reversible
/// until the caller commits. Destinations are snapshotted first so a later
/// failure puts the profile back exactly as it was.
pub fn stage_copy_vpks_with_prefix(
    source_dir: &Path,
    destination_dir: &Path,
    mod_id: &str,
    selected: Option<&[String]>,
    ...
    for source in &sources {
        ...
        if let Err(error) = validate_original_vpk_name(file_name) {
            return Err(snapshot.rollback(error));
        }
        let dest = destination_dir.join(format!("{mod_id}_{file_name}"));
        if let Err(error) = snapshot.capture(&dest) {
            return Err(snapshot.rollback(error));
        }
        snapshot.track(dest);
    }

    match copy_resolved_vpks_with_prefix(&sources, destination_dir, mod_id) {
        Ok(copied) => Ok(PendingVpkOperation::with_snapshot(copied, snapshot)),
...
fn resolve_copy_sources(
    ...
    if let Some(selected) = selected {
        let mut wanted: HashSet<String> = selected
            .iter()
        ...
        sources.retain(|path| {
            path.strip_prefix(source_dir)
                ...
                .is_ok_and(|relative| wanted.remove(&relative))
        });
        if !wanted.is_empty() {
            let mut missing: Vec<String> = wanted.into_iter().collect();
            missing.sort();
            return Err(VpkManagerError::Vpk(format!(
                "Mod {mod_id}: selected VPK files are not in the extracted directory: {}",
                missing.join(", ")
            )));
        }
    }
...
fn copy_resolved_vpks_with_prefix(
    sources: &[PathBuf],
    destination_dir: &Path,
    mod_id: &str,
) -> Result<Vec<String>> {
...
const MAX_ARCHIVE_WALK_DEPTH: usize = 32;
...
fn collect_vpks(dir: &Path, found: &mut Vec<PathBuf>) -> Result<()> {
    collect_vpks_at(dir, found, 0)
}

fn collect_vpks_at(dir: &Path, found: &mut Vec<PathBuf>, depth: usize) -> Result<()> {
    if depth > MAX_ARCHIVE_WALK_DEPTH {
        return Err(VpkManagerError::Invalid(format!(
            "Archive directory tree exceeds {MAX_ARCHIVE_WALK_DEPTH} levels"
        )));
    }
    for entry in fs::read_dir(dir)? {
        ...
        let path = entry.path();
        if file_type.is_dir() {
            collect_vpks_at(&path, found, depth + 1)?;
...
mod tests {
    ...
    /// Callers delete the extracted directory and the archive once this returns,
    /// so a selection naming a file that is not there has to fail rather than
    /// install a partial mod.
    #[test]
    fn copying_rejects_a_selection_it_cannot_resolve_in_full() {
        let temp = tempfile::tempdir().unwrap();
        let extracted = temp.path().join("extracted");
        ...
        let err = copy_vpks_with_prefix(
            &extracted,
            &base,
            "650634",
            Some(&["first.vpk".to_string(), "gone.vpk".to_string()]),
        )
        .unwrap_err();

        assert!(err.to_string().contains("gone.vpk"), "{err}");
        assert!(!base.join("650634_first.vpk").exists());
    }
