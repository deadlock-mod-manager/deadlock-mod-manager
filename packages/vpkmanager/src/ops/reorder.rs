...
pub fn reorder_vpks_sharded(
    ...
    Ok(stage_reorder_vpks_sharded(ordered_mods, base)?
        .commit()
        .placements)
}
...
pub fn stage_reorder_vpks_sharded(
    ordered_mods: &[ShardAssignment],
    base: &Path,
) -> Result<PendingVpkOperation<ShardReorderOutcome>> {
    if !base.exists() {
...
fn place_everything(
    ...
    for (shard_index, source) in orphan_sources {
        let filename = naming::file_name_of(&source.to_string_lossy());
    ...
    for (origin, staged) in &orphan_staged {
        let (shard, names) = cursor.place_group(base, staging, std::slice::from_ref(staged))?;
        if let Some(new_name) = names.into_iter().next() {
            orphan_renames.insert(origin.clone(), (shard, new_name));
        }
    }
...
mod tests {
    ...
    /// A reorder renumbers enabled VPKs that no assignment claimed. Reporting
    /// where each one landed is what lets a caller's records follow their files
    /// instead of pointing at whichever file took the old name.
    #[test]
    fn unclaimed_vpks_are_renumbered_and_their_new_names_reported() {
        let temp = tempfile::tempdir().unwrap();
        let addons_path = addons_base(&temp);
        let addons_path = addons_path.as_path();
        write_vpk(addons_path, "pak01_dir.vpk");
        ...
        let outcome =
            stage_reorder_vpks_sharded(&[assignment("claimed", "pak01_dir.vpk")], addons_path)
                .unwrap()
                .commit();

        assert_eq!(
            outcome.placements[0].vpks,
            vec!["pak01_dir.vpk".to_string()]
        );
        assert_eq!(
            outcome.orphan_renames,
            BTreeMap::from([(
                (ShardIndex::FIRST, "pak05_dir.vpk".to_string()),
                (ShardIndex::FIRST, "pak02_dir.vpk".to_string())
            )])
        );
        assert!(addons_path.join("pak02_dir.vpk").is_file());
    }
