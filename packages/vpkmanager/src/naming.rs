...
/// disabled mods.
///
/// Fails once the directory is full rather than naming a file above
/// [`SHARD_CAPACITY`](crate::profile::SHARD_CAPACITY), which the engine would
/// silently never load.
pub fn next_free_enabled_vpk_name(dir: &Path) -> Result<String> {
    ...
    let capacity = crate::profile::SHARD_CAPACITY;
    (1..=capacity)
        .find(|number| !used.contains(number))
        .map(enabled_vpk_name)
        .ok_or_else(|| {
            VpkManagerError::Vpk(format!(
                "{} already holds the {capacity} VPK files the engine loads from one addon folder",
                dir.display()
            ))
        })
}
...
mod tests {
    ...
    /// Staging recovery used to reach for `pak100_dir.vpk` here, which the
    /// engine ignores — the mod would look installed and never load.
    #[test]
    fn next_free_name_refuses_to_go_past_the_shard_capacity() {
        let temp = tempfile::tempdir().unwrap();
        for number in 1..=crate::profile::SHARD_CAPACITY {
            fs::write(temp.path().join(enabled_vpk_name(number)), b"vpk").unwrap();
        }

        let err = next_free_enabled_vpk_name(temp.path()).unwrap_err();
