...
pub enum StampReason {
    ...
    UnreadableStamp,
    /// Carries a stamp written by a newer build. Must not be overwritten.
    ForeignVersion,
}
...
pub struct DriftReport {
    ...
    /// Mods that no longer have a single file anywhere. Their entries stay so
    /// install order is not lost; they are marked disabled instead.
    pub forgotten_mods: BTreeSet<String>,
    /// Whether `ledger.files` or `ledger.mods` was mutated, including stat-cache
    /// updates that do not count as drift.
...
impl DriftReport {
    ...
    pub fn changed_ledger(&self) -> bool {
        self.ledger_dirty || !self.is_clean()
    }
...
pub fn reconcile(
    ...
    for file in scanned {
        ...
        let identity = match cached {
            ...
            None => match fingerprint::inspect(&file.path) {
                Ok(fingerprint::StampInspect::Present(found)) => Some((
                    found.id,
                    found.mod_id,
                    found.original_name,
                    found.content_hash,
                )),
                Ok(fingerprint::StampInspect::Absent) => None,
                Ok(fingerprint::StampInspect::ForeignVersion) => {
                    pending.push(PendingFile {
                        file,
                        reason: StampReason::ForeignVersion,
                        inherited: None,
                    });
                    continue;
                }
...
fn repair_mod(
        ...
        match &file.kind {
            VpkKind::Enabled { number } => {
                let claimed_here = entry.enabled
                    ...
                    && entry.current_vpks.contains(&file.filename);
                if claimed_here {
    ...
    enabled.sort_by_key(|entry| (entry.0, entry.1));
    disabled.sort();
    disabled.dedup();

    if enabled.is_empty() && disabled.is_empty() {
        // Nothing of this mod is left on disk. Keep the entry (and its install
        // order) so a transiently empty folder cannot wipe the user's load
        // order; uninstall is the only path that deletes a mod.
...
mod tests {
    ...
    fn a_deleted_vpk_disables_the_mod_that_owned_it() {
        ...
        let second = reconcile(&base, &mut ledger, Trust::Stat).unwrap();
        assert!(second.is_clean(), "{second:?}");
        assert!(second.forgotten_mods.is_empty());
    }
    ...
    #[test]
    fn an_entry_with_no_files_at_all_keeps_its_install_order() {
        let temp = tempfile::tempdir().unwrap();
        let base = base(&temp);
        let mut ledger = enabled_ledger("650634", "pak01_dir.vpk", "cool_mod.vpk");
    ...
    #[test]
    fn a_newer_version_stamp_is_left_alone() {
        let temp = tempfile::tempdir().unwrap();
        let base = base(&temp);
        let path = base.join("pak01_dir.vpk");
        unstamped_vpk(&path);
        let fingerprint = fingerprint::stamp(&path, "650634", "cool_mod.vpk").unwrap();
        fingerprint::write_foreign_stamp(&path, fingerprint).unwrap();
        let mut ledger = ProfileLedger::default();

        let report = reconcile(&base, &mut ledger, Trust::Stat).unwrap();

        assert_eq!(report.pending_stamps.len(), 1);
        assert_eq!(report.pending_stamps[0].reason, StampReason::ForeignVersion);
        assert!(ledger.files.is_empty());
    }
