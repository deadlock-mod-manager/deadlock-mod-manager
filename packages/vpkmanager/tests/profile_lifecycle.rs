...
fn install(base: &ProfileBase, mod_id: &str, extracted: &Path) -> Vec<String> {
    ...
    let enabled = ops::enable_vpks_in(base, base, mod_id, &prefixed)
        .unwrap()
        .commit();
...
fn deleting_a_file_by_hand_removes_the_mod_from_the_ledger() {
    ...
    let entry = ProfileLedger::load(base.path()).unwrap().mods["650634"].clone();
    assert!(!entry.enabled);
    assert_eq!(entry.order, Some(0));
}

/// A failed ledger write after enable must put every file back on its exact
/// original path, not merely some valid enabled slot.
#[test]
fn a_failed_ledger_save_after_enable_restores_the_original_filenames() {
    let temp = tempfile::tempdir().unwrap();
    let base = profile(&temp);
    let extracted = downloaded_mod(temp.path(), &["cool_mod.vpk"]);
    let prefixed = ops::copy_vpks_with_prefix(&extracted, &base, "650634", None).unwrap();

    let pending = ops::enable_vpks_in(&base, &base, "650634", &prefixed).unwrap();
    assert!(base.join("pak01_dir.vpk").is_file());
    let _ = pending.rollback(vpkmanager::VpkManagerError::Invalid(
        "simulated ledger save failure".to_string(),
    ));
