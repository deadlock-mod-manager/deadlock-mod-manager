...
export const modDownloads = pgTable('mod_download', {
  id: text('id').primaryKey().default(sql`concat('mod_download_', gen_random_uuid())`),
  ...
  return {
    unq: uniqueIndex('mod_download_mod_id_remote_id_idx').on(table.modId, table.remoteId),
    modIdIdx: uniqueIndex('mod_download_mod_id_idx').on(table.modId),
  };
});

export const customSettings = pgTable('custom_setting', {
  id: text('id').primaryKey().default(sql`concat('custom_setting_', gen_random_uuid())`),
