...
type DeadworksRegistryServer = z.output<typeof DeadworksRegistryServerSchema>;
...
const normalizeServers = (
  servers: DeadworksRegistryServer[],
  sourceUrl: string,
): ServerBrowserEntry[] => {
  return servers.flatMap((raw): ServerBrowserEntry[] => {
...
export const normalizeDeadworksRegistryServers = (
  ...
  return normalizeServers(payload.servers, sourceUrl);
};
