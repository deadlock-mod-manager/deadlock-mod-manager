...
import { z } from "zod";
import { INDEX_NAME, vectorStore, embeddingModel } from "./vector-store";
...
export const searchDocsTool = createTool({
  ...
  inputSchema: z.object({
    query: z.string().describe("Natural language search query about the docs"),
    topK: z
      .number()
      .min(1)
      .max(15)
  ...
  execute: async ({ query, topK }) => {
    ...
    const indexExists = (await vectorStore.listIndexes()).includes(INDEX_NAME);
    if (!indexExists) {
      return { results: [] };
    }
